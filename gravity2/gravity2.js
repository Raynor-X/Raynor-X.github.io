...
class Simulation {
	...
	update() {
		...
		this.ctx.translate(this.pxdimensions.width * 0.5, this.pxdimensions.height * 0.5);
		...
		this.bodies.forEach((bodyX) => {
			...
			this.bodies.forEach((bodyY) => {
				...
				if (sqD < lineThreshold) {
					this.ctx.beginPath();
					this.ctx.strokeStyle = `rgba(255,255,255,${(lineThreshold - sqD) / lineThreshold * 0.5})`;
	...
	get yearsPerFrame() {
		return this._yearsPerFrame || 0.2;
	}
}
...
class Body {
	...
	solve() {
		...
		if (this.position.x > this.dimensions.x * 0.5 + 30) {
			this.position.x = this.dimensions.x * -0.5 - 10;
		} else if (this.position.x < this.dimensions.x * -0.5 - 30) {
			this.position.x = this.dimensions.x * 0.5 + 10;
		}
		if (this.position.y > this.dimensions.y * 0.5 + 30) {
			this.position.y = this.dimensions.y * -0.5 - 10;
		} else if (this.position.y < this.dimensions.y * -0.5 - 30) {
			this.position.y = this.dimensions.y * 0.5 + 10;
		}
	}
	...
	get yearsPerFrame() {
		return this._yearsPerFrame || 0.008;
	}
}
